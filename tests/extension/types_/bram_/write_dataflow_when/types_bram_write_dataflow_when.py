...
def mkMain(n=128, datawidth=32, numports=2):
    ...
    mybram = bram.Bram(m, 'mybram', clk, rst, datawidth, addrwidth)
    ...
    waddr = 0
    wlen = 32
    done = mybram.write_dataflow(wport, waddr, a, wlen, cond=fsm, when=c)
    ...
    seq.If(mybram[0].wenable)(
        Systask('display', '[%d] <- %d', mybram[0].addr, mybram[0].wdata)
    )
