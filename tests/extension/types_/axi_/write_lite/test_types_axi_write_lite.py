...
(
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if(myaxi_wvalid && myaxi_wready && !(myaxi_bvalid && myaxi_bready)) begin
        outstanding_wreq_count_0 <= outstanding_wreq_count_0 + 1;
      end 
      if(!(myaxi_wvalid && myaxi_wready) && (myaxi_bvalid && myaxi_bready) && (outstanding_wreq_count_0 > 0)) begin
        outstanding_wreq_count_0 <= outstanding_wreq_count_0 - 1;
      end 
      myaxi_araddr <= 0;
      myaxi_arvalid <= 0;
      ...
      if((fsm == 1) && (myaxi_wready || !myaxi_wvalid)) begin
        myaxi_wdata <= wdata;
        myaxi_wvalid <= 1;
        myaxi_wstrb <= { 4{ 1'd1 } };
      end 
      ...
      if((fsm == 2) && (myaxi_awready || !myaxi_awvalid)) begin
        myaxi_awaddr <= 2048;
        myaxi_awvalid <= 1;
      end 
      _myaxi_cond_2_1 <= 1;
      ...
      if((fsm == 3) && (myaxi_wready || !myaxi_wvalid)) begin
        myaxi_wdata <= wdata;
        myaxi_wvalid <= 1;
        myaxi_wstrb <= { 4{ 1'd1 } };
      end 
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if(_seq_cond_0_1) begin
        $display("sum=%d expected_sum=%d", sum, 2049);
      end 
      if(myaxi_wvalid && myaxi_wready) begin
        sum <= sum + myaxi_wdata;
      end 
      _seq_cond_0_1 <= myaxi_wvalid && myaxi_wready;
    end
  end
...
def test():
    veriloggen.reset()
    test_module = types_axi_write_lite.mkTest()
