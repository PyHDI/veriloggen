...
def mkLed():
    ...
    def body(size, offset):
        ...
        laddr3 = size + size
        gaddr3 = size * 8 + offset

        # there must be no clock cycle between following 3 lines
        myaxi.dma_write_async(myram0, laddr1, gaddr1, size)
        myaxi.dma_write_async(myram0, laddr2, gaddr2, size)
        myaxi.dma_write_async(myram0, laddr3, gaddr3, size)

        myaxi.dma_wait_write()

        # read
        laddr1 = size + size
        gaddr1 = size * 8 + offset
        laddr2 = size
        gaddr2 = size * 4 + offset
        laddr3 = 0
        gaddr3 = offset
        ...
        myaxi.dma_read_async(myram1, laddr3, gaddr3, size)
...
def mkTest(memimg_name=None):
    ...
    memory = axi.AxiMemoryModel(m, 'memory', clk, rst, memimg_name=memimg_name, write_delay=50)
