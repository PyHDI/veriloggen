  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if((th_blink == 4) && !myfifo_full) begin
        myfifo_wdata <= _th_blink_wdata_1 << 8;
      end 
      if((th_blink == 4) && !myfifo_full) begin
        myfifo_enq <= 1;
      end 
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      case(th_blink)
        ...
        th_blink_17: begin
          _th_blink_rdata_5 <= _tmp_fixed_0;
...
def test():
    veriloggen.reset()
    test_module = thread_fixed_fifo_try.mkTest()
