import lib_pipeline_single_passthrough
...
(
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if(((vx && rx) && (_pipe_ready_1 || (!_pipe_valid_1)))) begin
        _pipe_data_1 <= x;
      end 
...
def test():
    test_module = lib_pipeline_single_passthrough.mkTest()
