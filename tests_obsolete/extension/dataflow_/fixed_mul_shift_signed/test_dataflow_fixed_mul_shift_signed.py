...
import veriloggen
import dataflow_fixed_mul_shift_signed
...
(
  ...
  initial begin
    $dumpfile("dataflow_fixed_mul_shift_signed.vcd");
...
(
  ...
  wire signed [32-1:0] _dataflow_times_data_2;
  wire _dataflow_times_valid_2;
  wire _dataflow_times_ready_2;
  wire signed [64-1:0] _dataflow_times_mul_odata_2;
  reg signed [64-1:0] _dataflow_times_mul_odata_reg_2;
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if(_dataflow_times_ready_2 || !_dataflow_times_valid_2) begin
        _dataflow_times_mul_odata_reg_2 <= _dataflow_times_mul_odata_2 >>> 4;
      end 
      if(_dataflow_times_ready_2 || !_dataflow_times_valid_2) begin
...
def test():
    veriloggen.reset()
    test_module = dataflow_fixed_mul_shift_signed.mkTest()
