...
import veriloggen
import pipeline_single_passthrough
...
(
  ...
  reg _df_valid_0;
  wire _df_ready_0;
  assign _df_ready_0 = 1;
  reg [32-1:0] _df_data_1;
  reg _df_valid_1;
  wire _df_ready_1;
  assign rx = (_df_ready_0 || !_df_valid_0) && vx && ((_df_ready_1 || !_df_valid_1) && vx);
  assign y = _df_data_1;
  ...
  always @(posedge CLK) begin
    if(RST) begin
      ...
      if(vx && rx && (_df_ready_1 || !_df_valid_1)) begin
        _df_data_1 <= x;
      end 
      if(_df_valid_1 && _df_ready_1) begin
        _df_valid_1 <= 0;
      end 
      if(rx && (_df_ready_1 || !_df_valid_1)) begin
        _df_valid_1 <= vx;
      end 
    end
  end
...
def test():
    veriloggen.reset()
    test_module = pipeline_single_passthrough.mkTest()
