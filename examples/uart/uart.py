...
def mkUartTx(baudrate=19200, clockfreq=100*1000*1000):
    ...
    fsm.add( waitcount(waitnum-1), txd(1), mem(Cat(din, Int(0, 1))) )
    fsm.add( ready(0), cond=enable )
    fsm.goto_next(cond=enable)

    for i in range(10):
        fsm.add(txd(mem[0]), mem(Cat(Int(1, 1), mem[8:1])), waitcount(waitnum-1),
                cond=waitcount==0)
        fsm.add(waitcount.dec(), cond=waitcount>0)
        fsm.goto_next(cond=waitcount==0)
...
def mkUartRx(baudrate=19200, clockfreq=100*1000*1000):
    ...
    fsm.add( valid(0), waitcount(int(waitnum/2)-1), mem(Cat(rxd, mem[8:1])) )
    fsm.goto_next(cond=rxd==0)

    for i in range(10):
        if i == 0: # check the start bit again
            fsm.goto_init(cond= rxd!=0)
        fsm.add(mem(Cat(rxd, mem[8:1])), waitcount(waitnum-1),
                cond=waitcount==0)
        fsm.add(waitcount.dec(), cond=waitcount>0)
        fsm.goto_next(cond=waitcount==0)

    fsm.add(valid(1), dout(mem[8:0]))
    fsm.goto_init()
